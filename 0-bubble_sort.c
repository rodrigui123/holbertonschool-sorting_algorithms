...
/**
 * bubble_sort - sorts an array of integers in ascending order using the Bubble sort alg
 * @array: the list
 * @size: size of list
 * Return: Always 0
 */
	...
	size_t i = 0, j = 0, size_aux = size - 1;
	   int aux = 0;
         ...
         while (size_aux) */ mientras haya algo en la lista */
         {
                 ...
                 {
                         j = i;
                         if ((array[j] > array[j + 1]) && (j + 1) < size_aux) /* entra en este if, si el aux es menor a i/j */
                         {
                                 aux = arrayj[j];
                                 array[i] = array[j + 1];
                                 array[j + 1] = aux;
